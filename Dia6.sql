...
-- CONSULTAS INSERT ALL (UNICAS DE ORACLE)
insert all
...
-- NO PERMITE SUBCONSULTAS INSERT ALL PORQUE LO HACE  A LA VEZ ( METERIA EL MISMO ID EN ESTE CASO)
insert all
...
*/
select * from copiadeptdos;
describe copiadeptdos;

create table copiadeptdos 
as 
select copiadept.*,
case loc  
when 'SEVILLA' then 1
when 'MADRID' then 2
when 'BARCELONA' then 3
when 'GRANADA' then 4
end as numciudad from copiadept;

-- INSERT INTO
-- PERMITE COPIAR DATOS DE UNA TABLA ORIGEN A UNA TABLA DESTINO, LA DIFERENCIA CON CREATE SELECT ESTA TABLA DEBE DE EXISTIR 
-- SIN LA TABLA DE DESTINO NO PODEMOS EJECUTAR ESTA FUNCION
/*
...
--& UN ASPERSAN SOLICITA POR VENTANA EL TEXTO QUE IRA AHI
--&& DOS ASPERSAN SOLICITA Y GUARDA Y NO VUELVE A SOLICITARLO
select apellido,oficio,salario, comision from emp where emp_no = &&numpresidente ;


select * from plantilla natural join hospital,sala;

-- NECESITO SABER LOS EMPLEADOS PARA NEGRO (7698)
...
*/
select * from emp;
select level, emp.* from emp connect by prior emp_no = dir start with apellido = 'jimenez' order by level; -- DESCENDENTE
select level, emp.* from emp connect by emp_no = prior dir start with apellido = 'jimenez' order by level; -- ASCENDENTE
select level, emp.* from emp connect by prior emp_no = dir start with oficio = 'PRESIDENTE' order by level;
-- LISTADO DE LOS JEFES DE ARROYO
select level, emp.* from emp connect by emp_no = prior dir start with apellido = 'arroyo' order by level desc; -- ASCENDENTE
select level, emp.*,sys_connect_by_path(apellido,'|') as relacion from emp connect by prior emp_no = dir start with apellido = 'jimenez' order by level; -- DESCENDENTE
